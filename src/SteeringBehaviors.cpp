...
Vector2 SteeringBehaviors::Flee(Vector2 targetPos, double panicDistance)
{
    if (panicDistance > 0.0f)
    {
        ...
        if (m_pVehicle->Position().DistanceSq(targetPos) > panicDistanceSq)
        {
            return Vector2(0, 0);
        }
    }
    Vector2 desiredVelocity = Vector2::Normalize(m_pVehicle->Position() - targetPos) * m_pVehicle->MaxSpeed();
    return desiredVelocity - m_pVehicle->Velocity();
}

Vector2 SteeringBehaviors::Arrive(Vector2 targetPos, Deceleration deceleration)
{
    Vector2 toTarget = targetPos - m_pVehicle->Position();
...
{
    Vector2 toEvader = evader->Position() - m_pVehicle->Position();
    ...
    m_vPursuitTarget = evader->Position() + evader->Velocity() * lookAheadTime;
...
{
    Vector2 toPursuer = pursuer->Position() - m_pVehicle->Position();

    double lookAheadTime = toPursuer.Length() / (m_pVehicle->MaxSpeed() + pursuer->Speed());

    return Flee(pursuer->Position() + pursuer->Velocity() * lookAheadTime, 0.f);
}
...
Vector2 SteeringBehaviors::Wander()
{
    ...
    return targetWorld - m_pVehicle->Position();
}
...
{
    Vector2 toTarget = Vector2::Normalize(targetPosition - vehicle->Position());
